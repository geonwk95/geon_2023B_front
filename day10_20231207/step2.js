...
// 1. 상수만들기
    // 1. const         선언 키워드 작성     왜? 상수인지 변수인지 식별용    ex 남 vs 여
    // 2. 상수명        이름[식별자]    왜? 상수가 여러개 있으므로 그 중에서 구별 ex 남 중에서 '김현수'
    // 3. =             대입연산자      [ 값을 변수에 넣는 역할 ] 왜? 오른쪽값을 왼쪽에 대입
    // 4. 자료          저장할 값

const pi = 3.141592;    // 주기억장치에 저장 
// 2. 상수 호출하는 방법 [ 상수 이름 작성 ]
console.log( pi );  // 상수 출력

// 3. 상수의 값을 변경하는 방법은 없다.
// pi = 3.97;
...
// 5. 동일한 상수명 사용 [ 상수명은 식별용 이므로 중복 이 불가능]
//const pi = 3.97; 
...
// 2. 변수 호출 [ 변수 호출시에는 키워드[let,const] 안쓴다]
console.log( pi3 );
...
// p.107 var 와 let 키워드 차이
let value1 = 10;
// let value1 = 10; // 불가능 초기화1번
var value2 = 10;
...
// 예제1 : 2개의 숫자를 입력받아 더한 결과 값을 html에 <h3> 안에 출력 
    ...
    // 1. prompt("출력하고 싶은말")    : 입력[input] 제공하는 함수
        // input 입력된 결과를 반환[ 10+10 => 20 ]
let ex1Value1 = prompt("예제1: 첫번째 숫자를 입력하세요.");
        // 만약에 10 입력시
        // let ex1Value1 = 10;
console.log( typeof ex1Value1 );
        // 타입 변환1 / 문제점 : 혹여나 소수점이 있으면 사라짐 
//ex1Value1 = parseInt( ex1Value1 ); console.log( typeof ex1Value1 );
        // 타입 변환2 [ Number( "숫자" )] -> 숫자 ]
ex1Value1 = Number( ex1Value1); console.log( typeof ex1Value1);

let ex1Value2 = Number( prompt("예제1: 두번째 숫자를 입력하세요."));
    //          함수1( 함수() ); // 함수2 결과를 함수1 넣고 함수1 결과 얻는과정
...
// 예제2 : 2개의 숫자를 입력받아 더 큰수를 html에 <h3> 안에 출력    
     ...
     // 1. 입력받은 데이터를 숫자 자료형으로 변환[ 숫자로 연산할려고/문자로 연산할수는 없으니까 ]후 각 변수에 대입[ 기억했다가 다음 문장에서 사용할려고 ]
let ex2Value1 = Number( prompt("예제2: 첫번째 숫자를 입력하세요") );
let ex2Value2 = Number( prompt("예제2: 두번째 숫자를 입력하세요") );
console.log( ex2Value1 > ex2Value2 ); // 만약에 10 과 20을 입력했다면 false
    // 2. *
