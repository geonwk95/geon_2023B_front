...
// 2. .isNaN( 숫자 ) : 유효성검사 [ 자료형에 따라 연산이 달라짐. ]
const m = Number('숫자로 변환할 수 없는 경우');
console.log( m );
console.log( m === NaN );
console.log( Number.isNaN(m));

// 3. .isFinite( 연산 )  : 유효성검사 : 연산의 결과가 무한대 이면
const n = 10 / 0 ;
console.log( n );
...
// 3. JSON : 다른 애플리케이션과 통신할때 데이터 교환 목적 활용
    // 1. JS객체 --> JSON문자열 : JSON.stringify()
    // 2. JSON문자열 --> JS객체 : JSON.parse()

// p. 356 좀 더 알아보기2
// - [ JS가 꺼지고/사라져도 사라지지 않는 메모리 ]
    // - localStorage   : 브라우저[크로,엣지]에 저장 , 브라우저 자체가 꺼지면 사라짐 , 다른 탭에서 공유
    // - sessionstorage : 브라우저[크로,엣지]에 저장 , 탭 꺼지면 메모리도 사라짐.   , 같은 탭에서 공유
